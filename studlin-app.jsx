    ...
    <div style={{display:"flex",minHeight:"100vh"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {[
            ...
            <div key={i} style={{background:"rgba(174,206,94,0.06)",border:"1px solid rgba(174,206,94,0.10)",borderRadius:14,padding:"16px 18px"}}>
              ...
              <div style={{fontSize:12.5,color:"rgba(232,239,231,0.45)",marginTop:3,lineHeight:1.4}}>{c.sub}</div>
            </div>
          ))}
        </div>
        <div style={{position:"absolute",bottom:-60,left:-60,width:200,height:200,borderRadius:"50%",border:"1px solid rgba(174,206,94,0.08)",pointerEvents:"none"}}/>
        <div style={{position:"absolute",bottom:-30,left:-30,width:140,height:140,borderRadius:"50%",border:"1px solid rgba(174,206,94,0.06)",pointerEvents:"none"}}/>
      </div>
      ...
      <div style={{flex:1,background:"#F5F2EC",display:"flex",flexDirection:"column",alignItems:"center",padding:"40px 40px 32px",overflowY:"auto"}}>
        <div style={{alignSelf:"flex-end",fontSize:13,color:"#6B6B67",marginBottom:40}}>
          {isSI?<>New to Studlin? <button onClick={()=>{setMode("signup");setShowEmail(false);setError("");}} style={{background:"none",border:"none",color:"#7A9A2F",fontWeight:600,cursor:"pointer",fontFamily:F,fontSize:13,padding:0}}>Create an account</button></>
          :<>Already have an account? <button onClick={()=>{setMode("signin");setShowEmail(false);setError("");}} style={{background:"none",border:"none",color:"#7A9A2F",fontWeight:600,cursor:"pointer",fontFamily:F,fontSize:13,padding:0}}>Sign in</button></>}
        </div>

        <div style={{width:"100%",maxWidth:420,flex:1,display:"flex",flexDirection:"column",justifyContent:"center",animation:"studlinRise 0.35s ease"}}>
          <h2 style={{fontSize:30,fontWeight:700,color:"#1A1A1A",margin:"0 0 6px",textAlign:"center",letterSpacing:"-0.02em"}}>{isSI?"Log in to ":"Sign up for "}<span style={{fontFamily:"'Instrument Serif',Georgia,serif",fontStyle:"italic",color:"#4A6B1A"}}>Studlin</span></h2>
          <p style={{fontSize:14,color:"#8A8A86",textAlign:"center",margin:"0 0 32px"}}>{isSI?"Welcome back. Pick up right where you left off.":"Start studying smarter today. It's free."}</p>

          {!showEmail?(<>
            <button onClick={()=>socialSign(new firebase.auth.GoogleAuthProvider())} disabled={loading} style={{...providerBtn,border:"1px solid #D4D4D0",background:"#fff",color:"#1A1A1A"}}>
            ...
            <button onClick={()=>socialSign(new firebase.auth.OAuthProvider("apple.com"))} disabled={loading} style={{...providerBtn,border:"none",background:"#1A1A1A",color:"#fff"}}>
            ...
            <button onClick={()=>setShowEmail(true)} style={{...providerBtn,border:"1px solid #D4D4D0",background:"#fff",color:"#1A1A1A",marginBottom:0}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#6B6B67" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>
              Use email instead
            </button>
          </>):(<>
            <form onSubmit={submit}>
              {!isSI&&<input value={name} onChange={e=>setName(e.target.value)} placeholder="Full name" style={{width:"100%",padding:"13px 14px",borderRadius:10,border:"1px solid #D4D4D0",background:"#fff",color:"#1A1A1A",fontSize:14,fontFamily:F,outline:"none",marginBottom:10}} />}
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email address" required style={{width:"100%",padding:"13px 14px",borderRadius:10,border:"1px solid #D4D4D0",background:"#fff",color:"#1A1A1A",fontSize:14,fontFamily:F,outline:"none",marginBottom:10}} />
              <input type="password" value={pass} onChange={e=>setPass(e.target.value)} placeholder="Password" required minLength={6} style={{width:"100%",padding:"13px 14px",borderRadius:10,border:"1px solid #D4D4D0",background:"#fff",color:"#1A1A1A",fontSize:14,fontFamily:F,outline:"none",marginBottom:10}} />
              {error&&<div style={{fontSize:12,color:"#dc2626",marginBottom:10,padding:"10px 12px",background:"#fef2f2",borderRadius:8,border:"1px solid #fecaca"}}>{error}</div>}
              <button type="submit" disabled={loading} style={{width:"100%",padding:"13px 16px",borderRadius:10,border:"none",background:"#1A2418",color:"#E8EFE7",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:F,opacity:loading?0.6:1}}>{loading?"...":(isSI?"Sign in":"Create account")}</button>
            </form>
          ...
          {error&&!showEmail&&<div style={{fontSize:12,color:"#dc2626",marginTop:10,padding:"10px 12px",background:"#fef2f2",borderRadius:8,border:"1px solid #fecaca",textAlign:"center"}}>{error}</div>}
        </div>

        <div style={{marginTop:32,textAlign:"center"}}>
          <div style={{height:1,background:"#D4D4D0",marginBottom:16}}/>
          <div style={{fontSize:12,color:"#8A8A86",marginBottom:8}}>Protected by industry-standard encryption. We never share your data.</div>
          <div style={{fontSize:12,color:"#8A8A86"}}>Choose a provider above, or use your email.</div>
          <div style={{fontSize:12,marginTop:10}}><a href="#" style={{color:"#7A9A2F",textDecoration:"none"}}>Privacy Policy</a> <span style={{color:"#C4C4C0",margin:"0 4px"}}>·</span> <a href="#" style={{color:"#7A9A2F",textDecoration:"none"}}>Terms of Service</a></div>
        </div>
      </div>
    </div>
  );
